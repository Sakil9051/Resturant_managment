...
<head>
    ...
    <title>Edit Ingredient - RestoAdmin</title>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white shadow-sm z-10">
                <div class="flex items-center justify-between px-8 py-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Update inventory details</p>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto bg-gray-50 p-8">
                <div class="bg-white rounded-xl shadow-sm p-8 max-w-2xl mx-auto">
                    <form action="<?= base_url('admin/inventory/update/' . $ingredient['id']) ?>" method="post">
                        <div class="mb-6">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Ingredient Name</label>
                            <input type="text" name="name" value="<?= esc($ingredient['name']) ?>" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                        </div>
                        <div class="grid grid-cols-2 gap-6 mb-6">
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Quantity</label>
                                <input type="number" step="0.01" name="quantity" value="<?= esc($ingredient['quantity']) ?>" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Unit</label>
                                <input type="text" name="unit" value="<?= esc($ingredient['unit']) ?>" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                            </div>
                        </div>
                        <div class="mb-8">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Min Threshold</label>
                            <input type="number" step="0.01" name="min_threshold" value="<?= esc($ingredient['min_threshold']) ?>" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                        </div>
                        <div class="flex justify-end space-x-4">
                            <a href="<?= base_url('admin/inventory') ?>" class="bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold py-2.5 px-6 rounded-lg transition-all">Cancel</a>
                            <button type="submit" class="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition-all transform hover:-translate-y-0.5">Update Ingredient</button>
                        </div>
                    </form>
                </div>
            </main>
