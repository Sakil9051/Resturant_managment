...
<head>
    ...
    <title>Edit Menu Item - RestoAdmin</title>
            ...
            <header class="bg-white shadow-sm z-10">
                <div class="flex items-center justify-between px-8 py-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Update menu details and pricing</p>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-8 max-w-2xl mx-auto">
                    <form action="<?= base_url('admin/menu/update/' . $item['id']) ?>" method="post" enctype="multipart/form-data">
                        <div class="mb-6">
                            ...
                            <input type="text" name="name" value="<?= esc($item['name']) ?>" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                        </div>
                        <div class="grid grid-cols-2 gap-6 mb-6">
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Category</label>
                                <select name="category_id" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="price" value="<?= esc($item['price']) ?>" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                            </div>
                        </div>
                        <div class="mb-6">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Description</label>
                            <textarea name="description" rows="4" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"><?= esc($item['description']) ?></textarea>
                        </div>
                        <div class="mb-6">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Image <span class="text-gray-400 font-normal">(Leave blank to keep current)</span></label>
                            <input type="file" name="image" class="border border-gray-300 rounded-lg w-full py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                        </div>
                        <div class="mb-8">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="available" value="1" <?= $item['available'] ? 'checked' : '' ?> class="form-checkbox h-5 w-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
                                <span class="ml-2 text-gray-700 text-sm font-bold">Available for Order</span>
                            </label>
                        </div>
                        <div class="flex justify-end space-x-4">
                            <a href="<?= base_url('admin/menu') ?>" class="bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold py-2.5 px-6 rounded-lg transition-all">Cancel</a>
                            <button type="submit" class="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition-all transform hover:-translate-y-0.5">Update Item</button>
                        </div>
                    </form>
                </div>
            </main>
